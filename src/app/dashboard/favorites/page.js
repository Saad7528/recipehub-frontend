...
import { Heart, Trash2, Eye, Clock, Loader2 } from "lucide-react";
...
export default function MyFavorites() {
  ...
  const handleRemove = async (id) => {
    try {
      const res = await fetch(`${apiBase}/api/favorites/${id}`, { method: "DELETE", credentials: "include" });
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white">My Favorites</h1>
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Recipes you have favorited or bookmarked for later cooking.</p>
      </div>
      ...
      {recipes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white border border-zinc-200 dark:border-zinc-800 dark:bg-zinc-900 rounded-3xl text-center px-4">
          ...
          <h3 className="text-lg font-bold text-zinc-700 dark:text-zinc-300">No favorites saved</h3>
          <p className="text-sm text-zinc-450 dark:text-zinc-550 mt-1 max-w-xs">
            Browse through recipes and click the star icon to save your favorite dishes.
          </p>
          <Link
            href="/recipes"
            ...
            >
              ...
              <div className="flex border-t border-zinc-100 dark:border-zinc-800 p-3 gap-2 bg-zinc-50/50 dark:bg-zinc-900/20">
                <Link
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-white border border-zinc-200 py-2.5 text-xs font-semibold text-zinc-600 hover:bg-zinc-50 dark:bg-zinc-950 dark:border-zinc-800 dark:text-zinc-350 dark:hover:bg-zinc-900"
                >
